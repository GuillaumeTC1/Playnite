...
using Playnite.Legacy;
using Playnite.Legacy.Common;
using Playnite.Legacy.Common.Extensions;
using Playnite.Legacy.Database;
using Playnite.Legacy.Database.Collections;
using Playnite.Legacy.Database.OldModels;
using Playnite.Legacy.Database.OldModels.NewVer1;
using Playnite.Legacy.Emulators;
        ...
        public static void MigrateNewDatabaseFormat(string databasePath)
        {
            ...
            if (dbSettings.Version == 1 && NewFormatVersion > 1)
            {
                ...
                foreach (var file in Directory.EnumerateFiles(gamesDir, "*.json"))
                {
                    ...
                    // Genres
                    convetList(game, nameof(OldGame.Genres), nameof(Ver2_Game.GenreIds), allGenres);

                    // Developers
                    convetList(game, nameof(OldGame.Developers), nameof(Ver2_Game.DeveloperIds), allCompanies);

                    // Publishers
                    convetList(game, nameof(OldGame.Publishers), nameof(Ver2_Game.PublisherIds), allCompanies);

                    // Tags
                    convetList(game, nameof(OldGame.Tags), nameof(Ver2_Game.TagIds), allTags);

                    // Categories
                    convetList(game, nameof(OldGame.Categories), nameof(Ver2_Game.CategoryIds), allCategories);

                    // Series
                    covertObject(game, nameof(OldGame.Series), nameof(Ver2_Game.SeriesId), allSeries);

                    // AgeRating
                    covertObject(game, nameof(OldGame.AgeRating), nameof(Ver2_Game.AgeRatingId), allRatings);

                    // Region
                    covertObject(game, nameof(OldGame.Region), nameof(Ver2_Game.RegionId), allRegions);

                    // Source
                    covertObject(game, nameof(OldGame.Source), nameof(Ver2_Game.SourceId), allSources);
