    ...
    public static class WindowUtils
    {
        ...
        public static void RestoreWindow(this Window window)
        {
            ...
            try
            {
                ...
                var thisWindowThreadId = User32.GetWindowThreadProcessId(interopHelper.Handle, nint.Zero);

                //Get the process ID for the foreground window's thread
                var currentForegroundWindow = User32.GetForegroundWindow();
                var currentForegroundWindowThreadId = User32.GetWindowThreadProcessId(currentForegroundWindow, nint.Zero);

                //Attach this window's thread to the current window's thread
                User32.AttachThreadInput(currentForegroundWindowThreadId, thisWindowThreadId, true);

                //Set the window position
                User32.SetWindowPos(interopHelper.Handle, new nint(0), 0, 0, 0, 0, SWP.NOSIZE | SWP.NOMOVE | SWP.SHOWWINDOW);
