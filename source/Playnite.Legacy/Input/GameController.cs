...
using static Playnite.Legacy.SDL;

namespace Playnite.Legacy.Input
{
    ...
    public class GameControllerManager : IDisposable
    {
        ...
        {
            public nint Controller { get; }
            public nint Joystic { get; }
            ...
            public LoadedGameController(nint controller, nint joystic, int instanceId, string path, string name)
            {
        ...
        private void SendKeyInput(uint key, bool pressed)
        {
            ...
            if (pressed)
            {
                User32.SendMessage(windowHandle, Winuser.WM_KEYDOWN, key, nint.Zero);
            }
            else
            {
                User32.SendMessage(windowHandle, Winuser.WM_KEYUP, key, nint.Zero);
            }
        }
