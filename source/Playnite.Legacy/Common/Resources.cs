...
namespace Playnite.Legacy.Common
{
    public class Resources
    {
        ...
        {
            nint hMod = Kernel32.LoadLibraryEx(path, nint.Zero, 0x00000002);
            nint hRes = Kernel32.FindResource(hMod, name, type);
            uint size = Kernel32.SizeofResource(hMod, hRes);
            nint pt = Kernel32.LoadResource(hMod, hRes);
        ...
        public static string GetIndirectResourceString(string fullName, string packageName, string resource)
        {
            ...
            var result = Shlwapi.SHLoadIndirectString(resourceString, sb, sb.Capacity, nint.Zero);
            if (result == 0)
            {
                return sb.ToString();
            }
            ...
            result = Shlwapi.SHLoadIndirectString(resourceString, sb, sb.Capacity, nint.Zero);
            if (result == 0)
            {
                return sb.ToString();
            }
